...
CLASS_ID_MASK = 1    
CLASS_ID_NO_MASK = 0
VECTORSTORE_PATH = "diagnosify_faiss_index"
...
def get_vectorstore():
    ...
    if os.path.exists(VECTORSTORE_PATH):
        print("🔹 Loading existing Diagnosify vector store...")
    ...
    print("🔹 Creating Diagnosify vector store for the first time...")

    loader = PyPDFLoader("diagnosify_data.pdf")
    ...
    print("✅ Diagnosify vector store saved.")
    return vectorstore
...
class AppointmentDetails(BaseModel):
    ...
    appointment_date: Optional[str] = Field(default=None, description="YYYY-MM-DD")
    time_slot: Optional[str] = Field(default=None, description="HH:MM")

# --- 1. UNIFIED STATE ---
class ChatState(TypedDict):
    messages: List[BaseMessage]
    intent: Optional[Literal["schedule", "doctor", "general", "out_of_scope"]]
...
def detect_master_intent(state: ChatState) -> ChatState:
    ...
    system_prompt = (
        ...
        "- 'schedule': User wants to schedule an appointment (even if details are missing).\n"
        "- 'doctor': User describes symptoms or asks for a suitable doctor.\n"
        "- 'general': Hospital-related questions not involving scheduling.\n"
        "- 'out_of_scope': Not related to hospitals or healthcare.\n\n"
...
# --- FIXED SCHEDULE GUARD ---
def schedule_guard(state: ChatState) -> ChatState:
    user_text = state["messages"][-1].content
    
    # 1. Get the real-time current date
    now = datetime.now()
    current_date_str = now.strftime("%A, %Y-%m-%d") 
    current_year = now.year

    # 2. Define the Prompt Instructions Explicitly
    extraction_instructions = (
        f"You are an expert appointment scheduler.\n"
        f"TODAY'S DATE is: {current_date_str}.\n"
        f"Current Year is: {current_year}.\n\n"
        "INSTRUCTIONS:\n"
        "1. Analyze the user's text to extract: Patient ID, Doctor ID, Date, and Time.\n"
        "2. Date Logic: If the user says '22 December' and today is Dec 21, assume it is this year (2025-12-22).\n"
        "3. Future Logic: If the user says 'January 5' and today is Dec 21, assume it is NEXT year (2026).\n"
        "4. Format Date as YYYY-MM-DD.\n"
        "5. Format Time as HH:MM (24-hour format). Convert '10 pm' to '22:00'.\n"
        "6. If a value is missing or unclear, return null (None) for that field."
    )

    # 3. Configure Structured Output (Without the system prompt param here)
    structured_llm = llm.with_structured_output(AppointmentDetails)

    try:
        # 4. Pass Instructions as SystemMessage + User Input
    ...
    # 5. Merge with existing state (Preserve old data if new extraction is None)
    if details.patient_id is not None: state["patient_id"] = details.patient_id
    if details.doctor_id is not None: state["doctor_id"] = details.doctor_id
    if details.appointment_date is not None: state["appointment_date"] = details.appointment_date
    if details.time_slot is not None: state["time_slot"] = details.time_slot

    # 6. Check for missing fields (Using 'is None' is safer for ID=0 scenarios)
    missing_fields = []
    if state.get("patient_id") is None: missing_fields.append("Patient ID")
    if state.get("doctor_id") is None: missing_fields.append("Doctor ID")
    if state.get("appointment_date") is None: missing_fields.append("Date")
    if state.get("time_slot") is None: missing_fields.append("Time")

    if missing_fields:
        state["success"] = False
        state["messages"].append(AIMessage(
            content=f"I can help schedule that. I just need the following details: {', '.join(missing_fields)}."
        ))
    else:
        state["success"] = True
...
def schedule_ready_router(state: ChatState) -> str:
...
def hospital_knowledge_node(state: ChatState) -> ChatState:
    user_text = state["messages"][-1].content

    # Retrieve relevant chunks
    docs = retriever.get_relevant_documents(user_text)

    # If nothing relevant → treat as out of scope
    if not docs:
        state["messages"].append(
            AIMessage(content="I can only help with hospital-related questions.")
        )
        return state
    ...
    system_prompt = (
        "You are Diagnosify, an intelligent hospital management assistant.\n"
        "Answer ONLY using the provided hospital context.\n"
        "If the answer is not in the context, say:\n"
        "'I can help only with Diagnosify hospital-related information.'\n\n"
        f"--- CONTEXT ---\n{context}"
    )
...
workflow.add_node("schedule_guard", schedule_guard)         # Extracts & Validates
workflow.add_node("schedule_appointment", schedule_appointment_node) # Database Action
...
workflow.add_node("hospital_knowledge", hospital_knowledge_node)
...
workflow.add_conditional_edges(
    ...
    route_by_intent,
    {
        "schedule_flow": "schedule_guard",
        "doctor_flow": "recommend_doctor",
        "general_flow": "hospital_knowledge",
        "out_of_scope_flow": "out_of_scope_node",
    },
)

# Schedule Routing (FIXED)
workflow.add_conditional_edges(
    "schedule_guard",
    schedule_ready_router,
    {
        # If success=True, go to DB node
        "schedule_appointment": "schedule_appointment", 
        # If success=False, End turn (bot asks user for missing info)
        END: END,
    },
)

# Connect specific nodes to END
...
workflow.add_edge("hospital_knowledge", END)
        ...
        if mask_model:
            ...
            for class_id, confidence in zip(detections.class_id, detections.confidence):
                ...
                if class_id == CLASS_ID_MASK:
                    found_safe = True
                if class_id == CLASS_ID_NO_MASK:
                    found_danger = True

            # Update Status Logic
            is_safe = found_safe and not found_danger
            status_state = "active" if (found_safe or found_danger) else "waiting"

            current_status["mask_detected"] = is_safe
            current_status["confidence"] = float(highest_conf * 100)
            current_status["status"] = status_state

            # Draw UI on the frame
            annotated_frame = box_annotator.annotate(scene=frame, detections=detections)
            labels = [f"{mask_model.names[cid]} {conf:0.2f}" for cid, conf in zip(detections.class_id, detections.confidence)]
...
def video_feed():
    # Returns the streaming video response
